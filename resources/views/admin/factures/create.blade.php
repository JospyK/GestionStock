    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.factures.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_facture') ? 'is-invalid' : '' }}" type="text" name="date_facture" id="date_facture" value="{{ old('date_facture') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date_reglement') ? 'is-invalid' : '' }}" type="text" name="date_reglement" id="date_reglement" value="{{ old('date_reglement') }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_locked') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_locked" id="is_locked" value="1" {{ old('is_locked', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Facture::ETAPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('etape') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="etape_{{ $key }}" name="etape" value="{{ $key }}" {{ old('etape', '') === (string) $key ? 'checked' : '' }}>
