...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.approvisionnement.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.approvisionnements.update", [$approvisionnement->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('article') ? 'is-invalid' : '' }}" name="article_id" id="article_id">
                    @foreach($articles as $id => $entry)
                        <option value="{{ $id }}" {{ (old('article_id') ? old('article_id') : $approvisionnement->article->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('num_serie') ? 'is-invalid' : '' }}" name="num_serie" id="num_serie">{{ old('num_serie', $approvisionnement->num_serie) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $approvisionnement->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('qte') ? 'is-invalid' : '' }}" type="number" name="qte" id="qte" value="{{ old('qte', $approvisionnement->qte) }}" step="0.01">
                @if($errors->has('qte'))
                    <div class="invalid-feedback">
                        {{ $errors->first('qte') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_ht') ? 'is-invalid' : '' }}" type="number" name="total_ht" id="total_ht" value="{{ old('total_ht', $approvisionnement->total_ht) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_tva') ? 'is-invalid' : '' }}" type="number" name="total_tva" id="total_tva" value="{{ old('total_tva', $approvisionnement->total_tva) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_ttc') ? 'is-invalid' : '' }}" type="number" name="total_ttc" id="total_ttc" value="{{ old('total_ttc', $approvisionnement->total_ttc) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Approvisionnement::ETAPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('etape') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="etape_{{ $key }}" name="etape" value="{{ $key }}" {{ old('etape', $approvisionnement->etape) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $approvisionnement->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
